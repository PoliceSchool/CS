...
int n ,m;
char G[MAXN][MAXN+1];			//记录原地图矩阵
bool mk[MAXN][MAXN];			//广搜用的已访问标记
map<pair<int,int>,int> Map;
int N;
int g[MAXG+2][MAXG+2];			//新图中点对剑的距离矩阵
int memo[MAXG+2][1<<MAXG];		//记忆化搜索用的记录已求解过的结果的表
struct Node{
	int r,c,d;
	Node(int r_ = 0,int c_ = 0,int d_ = 0):r(r_),c(c_),d(d_){}
};
...
int f(int ed,int mask){
	...
	for (int i = 0; i < N-2; i++){
		if (( mask & (1 << i))> 0 && i != ed){
			...
			memo[ed][mask] = memo[ed][mask] < t ? memo[ed][mask]:t;
		}
	}
	return memo[ed][mask];
}
